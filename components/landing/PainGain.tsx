          ...
          {STATS.map((s, i) => (
            ...
            >
              ...
              <div style={{ textAlign: "right" }}>
                <p style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.6px", color: "rgba(0,0,0,0.2)", marginBottom: 4, textDecoration: "line-through", textDecorationColor: "rgba(0,0,0,0.1)" }}>
                  {s.before}
                </p>
                <p style={{ fontSize: 12, color: "rgba(0,0,0,0.3)" }}>{s.label}</p>
              </div>

              {/* Arrow */}
              <svg width="20" height="16" viewBox="0 0 20 16" fill="none" style={{ flexShrink: 0 }}>
                <path d="M0 8h18M12 2l6 6-6 6" stroke="rgba(0,0,0,0.15)" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>

              {/* After */}
              <div>
                <p style={{ fontSize: 28, fontWeight: 600, letterSpacing: "-0.6px", color: "#059669", marginBottom: 4 }}>
                  {s.after}
                </p>
                <p style={{ fontSize: 12, color: "rgba(0,0,0,0.35)" }}>{s.label}</p>
              </div>
            </div>
          ))}
        </div>
